...
subtitle: "Simulations and real LD structure in contiguous SNPs (~90Kb)"
...
*****
# Aplication in a window region to explore high LD
...
ggplot(d) + 
  geom_vline(xintercept = which(eff.skewed!=0),lty="dashed",alpha=0.2)+
  geom_line(aes(y=abs(bgwa),x=position),color="black",alpha=0.5)+ 
  geom_line(aes(y=abs(b),x=position),color="red",alpha=0.5) +
  ylab(expression(paste("Absolute ",beta)))
...
For the last plot, which compares the discrete simulated effects (5% of SNPs have either -1 or +1 effect), the conditional analysis peaks coincide perfectly.
